...
* About
Pyems is a high-level, headless Python interface to the electromagnetic field
solver, [[https://openems.de/start/index.php][OpenEMS]]. It uses OpenEMS's own Python interface to hook into [[https://github.com/thliebig/CSXCAD][CSXCAD]]
...
Pyems focuses on being a robust translation and meshing engine. It is designed
to be easily integrated into automated design flows or driven by external CAD
tools via its declarative API.
...
3. A collection of low-level port classes (Microstrip, Coax, Waveguide)
   designed for reliable EM excitation and measurement.
